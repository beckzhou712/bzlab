...
/**
 * Directory segments a post sits under, relative to `BLOG_PATH`.
 * e.g. `src/content/posts/vida/first-results.md` → `["vida"]`
 *
 * Exported because column membership is derived from the first segment — the
 * column and the URL must come from the same computation or they can disagree.
 */
export function getPostPathSegments(filePath: string | undefined): string[] {
  return (
    filePath
      ?.replace(BLOG_PATH, "")
      .split("/")
